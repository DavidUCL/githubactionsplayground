...
const MUTATIONS = [
  ...
  ["db-assert: stop comparing the site identity", "scripts/db-assert.mjs",
    "`if($id !== '${identity}') exit(72); ` +\n    ", ""],
  ["db-assert: let Moodle swallow the exit code", "scripts/db-assert.mjs",
    "<?php define('CLI_SCRIPT',true); require(", "<?php require("],
  ["db-assert: let the other host skip the assertion", "scripts/db-assert.mjs",
    'return { step: "runPhpCode", code, critical: true };',
    'return { step: "runPhpCode", code, critical: false };'],
  ["db-assert: accept a siteidentifier too short to mean anything", "scripts/db-assert.mjs",
    "identity.length < MIN_IDENTITY || ", ""],
  ["db-assert: compare the versions without checking either side is there",
    "scripts/db-assert.mjs",
    "`if($branch === null || $dbbranch === false || $dbbranch === null || $dbbranch === '') exit(76); ` +\n    ",
    ""],
  ["db-assert: stop checking the database was adapted to this site", "scripts/db-assert.mjs",
    "`if($cfg('wwwroot') !== $CFG->wwwroot) exit(73); ` +\n    ", ""],
  ["snapshot: accept a file whose digest does not match", "scripts/snapshot.mjs",
    "expectedSha256 && sha256 !== expectedSha256.toLowerCase()", "false"],
  ["snapshot: accept anything that downloads as a database", "scripts/snapshot.mjs",
    'bytes.subarray(0, 16).toString("latin1") !== SQLITE_MAGIC', "false"],
  ["snapshot: ignore a snapshot from a different Moodle", "scripts/snapshot.mjs",
    "want && facts.branch && facts.branch !== want", "false"],
  // The subtle one. The published snapshot carries `additionalhtmlhead` as an
  // EMPTY row, so a refusal keyed on the row EXISTING rejects the very file
  // this control was built against — while looking completely correct.
  ...
  ["preview: reserve only the accounts this preview happens to make",
    ...
    "    reservedUsernames: accountNames(1, 1)"],
  ["preview: land on a numbered course under a restored database",
    "scripts/build-preview.mjs",
    "      restored: Boolean(restore) || Boolean(dbSnapshot),", "      restored: Boolean(restore),"],
  ["preview: print the renamed-admin caveat when nothing was renamed",
    "scripts/build-preview.mjs",
    '      (dbSnapshot && dbSnapshot.facts.adminUsername !== "admin"',
    "      (dbSnapshot"],
  ["preview: name `admin` in the brief instead of the snapshot's administrator",
    "scripts/build-preview.mjs",
    '    dbSnapshot ? dbSnapshot.facts.adminUsername : "admin",', '    "admin",'],
  ["preview: leave the restored administrator with the source site's password",
    "scripts/build-preview.mjs",
    '        { step: "setAdminAccount", password: "password", critical: true }]',
    "        ]"],
  ["preview: describe a database restore as code being rewritten",
    "scripts/build-preview.mjs",
    'const code = list.filter((r) => r !== "restoreDatabase" && r !== "restoreCourse");',
    "const code = list;"],
  ["preview: skip the database assertion", "scripts/build-preview.mjs",
    "    ...(dbSnapshot\n      ? [buildDatabaseAssertion({", "    ...(false\n      ? [buildDatabaseAssertion({"],
  ["preview: let the other host carry on past a failed database restore",
    "scripts/build-preview.mjs",
    '? [{ step: "restoreDatabase", url: dbSnapshot.url, critical: true }]',
    '? [{ step: "restoreDatabase", url: dbSnapshot.url }]'],
  ...
  ["login: ignore the login-as override", "scripts/build-preview.mjs",
    ...
    "const derivedUser = previewUser(landing, teachers);"],
  ...
  ["courseid: assume 2 instead of deriving it", "scripts/build-preview.mjs",
    "  const made = [];", "  return 2;\n  const made = [];"],
  ["courseid: accept a blueprint that creates no course at all", "scripts/build-preview.mjs",
    "  if (made.length === 0) {", "  if (false) {"],
  ["courses: accept REVIEW being created twice (second aborts on shortnametaken)",
    "scripts/build-preview.mjs",
    "  if (made.filter((n) => n === COURSE_SHORTNAME).length !== 1) {", "  if (false) {"],
  ["courseid: let a mod landing default its course number", "scripts/build-preview.mjs",
    "      if (!Number.isInteger(courseId)) {", "      if (false) {"],
  ...
  // --- the courses control ---
  ["courses: ignore the count and always build one", "scripts/build-preview.mjs",
    "  const extraCourses = extraCourseNames(courses);",
    "  const extraCourses = [];"],
  ["courses: create the extras BEFORE the review course (moves its id)",
    "scripts/build-preview.mjs",
    "    ...(extraCourses.length\n      ? [\n          { step: \"createCategory\", name: EXTRA_CATEGORY, critical: true },",
    "    ...([]\n      ? [\n          { step: \"createCategory\", name: EXTRA_CATEGORY, critical: true },"],
  ["courses: let the extras take the handler's default format and sections",
    "scripts/build-preview.mjs",
    "            format: activeFormat,\n            numsections: sections,\n          })),",
    "          })),"],
  ["courses: enrol everyone in every course (hides an enrolment-blind plugin)",
    "scripts/build-preview.mjs",
    "        ...(extraCourses.length\n          ? teacherNames(teachers).map((u) => ({\n              username: u,\n              course: extraCourses[0],",
    "        ...(extraCourses.length\n          ? extraCourses.flatMap((c) => teacherNames(teachers).map((u) => ({\n              username: u,\n              course: c,"],
  ["courses: stop requiring the review course to be first", "scripts/build-preview.mjs",
    "  if (made[0] !== COURSE_SHORTNAME) {", "  if (false) {"],
  ...
  ["courses: check enrolment as a flat set again, not per course",
    "scripts/build-preview.mjs",
    "  const inReview = new Set(\n    enrolments.filter((e) => e.course === COURSE_SHORTNAME).map((e) => e.username),\n  );",
    "  const inReview = new Set(enrolments.map((e) => e.username));"],
  ["courses: print a count instead of the roster", "scripts/build-preview.mjs",
    "      courses: courseRoster.join(\", \"),", "      courses: `${courseRoster.length} course(s)`,"],
  ["courses: ignore the plural createCourses step when finding the first course",
    "scripts/build-preview.mjs",
